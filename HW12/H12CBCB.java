// H12CBCB.java CS6025 Yizong Cheng February 2015
// Implementing AES decryption with CBC mode, IV = 0
// Usage: java H12CBCB key < encrypted > decrypted

import java.io.*;
import java.util.*;

public class H12CBCB
{
    ...
    void decrypt()
    {
        int[][] cipherTextBlock = new int[2][blockSize];

        if ( readBlock() <= 0 ) {
            return;
        }

        copyBlock( cipherTextBlock[0], state );
        blockDecipher();
        writeBlock();

        while ( readBlock() > 0 ) {
            copyBlock( cipherTextBlock[1], state );
            blockDecipher();
    ...
    {
        if ( args.length < 1 ) {
            System.err.println( "Usage: java H12CBCB key < encrypted > decrypted" );
            return;
        }

        H12CBCB h12 = new H12CBCB();
        h12.makeLog();
        h12.buildS();
        h12.readKey( args[0] );
        h12.expandKey();
